...
private fun innerCurrentFocusedMethod(context: IEnginesContext?, verbose: Boolean = true): IDexMethod? =
    with(unsafeGetGraphicalContext(context)) {
...
private fun innerCurrentFocusedType(context: IEnginesContext?, verbose: Boolean = true): IDexType? =
    with(unsafeGetGraphicalContext(context)) {
        val fragment = focusedFragment
        var address = fragment?.activeAddress
        if (fragment == null || address == null) {
...
fun currentFocusedMethod(context: IEnginesContext?, verbose: Boolean = true): IDexMethod? =
    runOnUi { innerCurrentFocusedMethod(context, verbose) }

fun currentFocusedType(context: IEnginesContext?, verbose: Boolean = true): IDexType? =
    runOnUi { innerCurrentFocusedType(context, verbose) }

private inline fun <reified T> runOnUi(crossinline f: () -> T): T? {
    val storage = arrayOfNulls<T>(1)
    Display.getDefault().syncExec {
        storage[0] = f()
        logger.error("sends: ${storage[0]}")
    }
    logger.error("received: ${storage[0]}")
    return storage[0]
}